...
/** Both methods here wrap an existing action block by first checking for existence of a
  * valid JWT in a cookie. If the JWT is valid then check the roles given in the
...
trait RolesSecuredBlocks extends play.api.Logging:
  ...
  def withRoles(
    ...
    if matchingRoles.equals(roles) then block(request)
    else
      logger.warn(s"Roles mismatch, received: ${request.user.roles}, required: $roles")
      Forbidden

  type AuthenticatedAsyncBlock[A] =
    AuthenticatedRequest[A, Authentication] => Future[Result]
  def withRolesAsync[A](
      roles: Set[String]
  )(block: AuthenticatedAsyncBlock[AnyContent]): AuthenticatedAsyncBlock[AnyContent] =
    request =>
      val matchingRoles = request.user.roles intersect roles
      if matchingRoles.equals(roles) then block(request)
      else
